...
const BookingInquirySchema = z.object({
  ...
  weddingDate: z
    .string()
    .min(1, { message: "Por favor, selecciona una fecha para la boda." }),
  venue: z
    .string()
...
export async function handleBookingInquiry(data: BookingInquiryData) {
  ...
  const availablePassword =
    process.env.SMTP_PASS || process.env.GMAIL_APP_PASSWORD;
  if (!availablePassword) {
    console.error(
      "No se encontró la variable de entorno SMTP_PASS ni GMAIL_APP_PASSWORD",
    );
    return {
      success: false,
  ...
  const smtpUser =
    ...
    "alex.montalvo.carrasco@gmail.com";
  ...
  if (smtpHost) {
    console.log(
      "Usando transporte SMTP desde variables de entorno (SMTP_HOST detectado)",
    );
    ...
    console.log(
      "No se detectó SMTP_HOST — usando configuración temporal de Gmail. Es recomendable configurar SMTP_HOST para enviar desde info@reveliophotography.es",
    );
    transporter = nodemailer.createTransport({
      host: "smtp.gmail.com",
      port: 465,
...
`;
  ...
  const mailFromAddress =
    process.env.MAIL_FROM || smtpUser || "alex.montalvo.carrasco@gmail.com";
  ...
  try {
    ...
    try {
      const isAddressString = (addr: string | nodemailer.Address): string => {
        return typeof addr === "string" ? addr : addr.address;
      };

      const acceptedEmails = (info.accepted || []).map(isAddressString);
      const rejectedEmails = (info.rejected || []).map(isAddressString);

      if (acceptedEmails.includes("info@reveliophotography.es")) {
        console.log(
          "La dirección info@reveliophotography.es fue ACCEPTED por el servidor SMTP",
        );
      } else if (rejectedEmails.includes("info@reveliophotography.es")) {
        console.warn(
          "La dirección info@reveliophotography.es fue REJECTED por el servidor SMTP",
        );
      } else {
        console.log(
          "No hay confirmación explícita de aceptación/rechazo para info@reveliophotography.es (puede depender del proveedor)",
        );
      }
    } catch (err) {
    ...
    return {
      success: true,
      message:
        "¡Gracias por vuestro mensaje! Me pondré en contacto con vosotros muy pronto.",
    };
  } catch (error: any) {
    ...
    if (error.code === "EAUTH") {
      ...
      console.error(
        "Error de conexión - Verifica la configuración del servidor SMTP",
      );
    }
